...
class CategoryConditionedDiffusion(pl.LightningModule):
    def __init__(self, vae_config_path, vae_weights_path, lr=1e-4, weight_decay=1e-4):
        ...
        self.sigma_data = 1.0  # EDM 원본과 동일
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(
            ...
            lr=self.lr,
            weight_decay=0.05  # 원본 VecSet과 동일
        )
        # Cosine annealing with warmup (원본 VecSet 학습 스케줄)
        from torch.optim.lr_scheduler import CosineAnnealingLR, LinearLR, SequentialLR
        warmup = LinearLR(optimizer, start_factor=1e-3, total_iters=40)
        cosine = CosineAnnealingLR(optimizer, T_max=self.trainer.max_epochs - 40, eta_min=1e-6)
