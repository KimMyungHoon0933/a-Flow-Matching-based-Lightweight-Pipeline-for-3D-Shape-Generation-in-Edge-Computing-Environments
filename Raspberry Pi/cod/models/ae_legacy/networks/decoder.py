...
from ..modules.blocks import StandardTransformer, CrossTransformer, init_embedding, GEGLU
...
class TriplaneDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_layers: int,
                 num_prune_layers: int,

                 keep_ratio: float = 0.5,
                 use_input_ln: bool = False,
                 num_register: int = 1,
                 use_mask_token: bool = False,
                 uncertainty_activation: str = 'sigmoid',
                 uncertainty_mode: str = 'mult',
                 num_merged_tokens: int = -1,
                 detach_uncertainty: bool = False,
                 use_prune_ffn: bool = True,
                 use_conv_refine: bool = False,
                 use_conv_init_refine: bool = False,

                 ## new param for uncertainty add
                 uncertainty_multiply: bool = True,
        ...
        self.use_conv_init_refine = use_conv_init_refine

        ## new params. Removed previous 'mult_uncertainty' b/c it was not used
        self.uncertainty_multiply = uncertainty_multiply

        self.prune_decoder = CrossTransformer(embed_dim=embed_dim, num_layers=num_prune_layers,
                                              use_ln=use_input_ln, num_register=num_register,
                                              use_ffn=use_prune_ffn,
                                              use_self_attn=use_prune_ffn,
                                              num_heads=num_heads, mlp_ratio=mlp_ratio, dropout=prune_dropout)
        self.decoder = StandardTransformer(embed_dim=embed_dim, num_layers=num_layers,
                                           use_ln=True, num_register=num_register,
        ...
        ## parameters
        self.mask_token = nn.Parameter(torch.empty(1, embed_dim)) if use_mask_token else None
    ...
    def decode(self, z: torch.Tensor):
        ...
        if self.uncertainty_multiply:
            patches = init_patches + uncertainty * patches
        else:
            patches = init_patches + patches
    ...
    def decode_tokens(self, tokens, z):
        ## prune decoder
        init_tokens = self.prune_decoder(tokens, z)
        uncertainty_inputs = init_tokens
        if self.detach_uncertainty:
            uncertainty_inputs = uncertainty_inputs.detach()
        uncertainty = self.uncertainty_out(uncertainty_inputs)
        ...
        tokens, z = self._run_transformer(self.decoder, tokens, z)
        tokens = tokens[:, :L]
    ...
    def _run_transformer(self, transformer, tokens, z):
        L = tokens.size(1)
    ...
    def decode_queries(self, planes: torch.Tensor, queries: torch.Tensor) -> torch.Tensor:
        ...
        query_features = _decode_queries_with_plane(query_features, planes, queries)
    ...
    def decode_uncertainty(self, planes, queries):
        ...
        # uncertainty = _decode_queries_with_plane(uncertainty, planes, queries, mode=self.uncertainty_mode)

        if self.uncertainty_mode == 'mult':
            uncertainty = torch.ones(queries.size(0), queries.size(1), 1, device=queries.device, dtype=queries.dtype)
            uncertainty = _decode_queries_with_plane(uncertainty, planes, queries, mode=self.uncertainty_mode)
        elif self.uncertainty_mode == 'avg':
            uncertainty = torch.zeros(queries.size(0), queries.size(1), 1, device=queries.device, dtype=queries.dtype)
            uncertainty = _decode_queries_with_plane(uncertainty, planes, queries, mode='sum')
        else:
    ...
    def apply_uncertainty_activation(self, x: torch.Tensor):
        if self.uncertainty_activation == 'sigmoid':
            x = F.sigmoid(x)
        elif self.uncertainty_activation == 'exp':
            x = torch.exp(x)
        elif self.uncertainty_activation == 'elu':
            x = F.elu(x) + 1
        elif self.uncertainty_activation == 'softmax':
            x = F.softmax(x, dim=1)
        elif self.uncertainty_activation == 'conf':
            return torch.exp(x).clamp(0.1, 10)
        elif self.uncertainty_activation == 'none':
            pass
        else:
            raise NotImplementedError
